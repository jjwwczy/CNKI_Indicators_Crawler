...
| 列名 | 含义 | 示例 | 备注 |
| :--- | :--- | :--- | :--- |
| `zcode` | 指标代码 | `Z017` | 从 Zcode 附表中选择，这里是“中国教育与经济社会发展统计数据库”在知网网站中的代码。`实测发现zcode不影响返回结果，搜索的时候还是会跨数据库返回结果，所以完全可以不改这个参数` |
...
下表列出了部分 `zcode` 及其对应的数据库名称，可以手动改网址后缀的zcode数字进入查看https://data.cnki.net/trade/home?zcode=Z017
